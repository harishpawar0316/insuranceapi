...
exports.SendOnlineUsers = async (io, Roomid, username) => {
  try {
    ...
    if (clientsInRoom) {
      ...
      for (const clientId of clientsInRoom) {
        ...
        if (query["usertype"] !== "ws_login" && query["usertype"] !== "client") {

          let data = await userTypeModels.findById(query["usertype"])
          if (data) query["usertype"] = data["usertype"]
        }
...
const FireBasePushNotification = async (
  ...
  try {
    ...
    if (eventname === "joinRoom") {
      ...
      if (usertype == "client") {
        socket.join(roomid);
        this.SendOnlineUsers(io, roomid, username);
...
const FireBasePushNotificationTransferChat = async (
  data,
  io,
  socket,
) => {
  try {
    ...
    io.to(data.roomid).emit("getTransferChat", roomobj);
  } catch (error) {
    console.error(error);
  }
};
module.exports = {
  transferChat: async (socket, io) => {
    try {
      ...
      // Join Room
      socket.on("transferChat", async (data) => {
        // send firebase cloud notification to all agents
        await FireBasePushNotificationTransferChat(
          data,
          io,
          socket,
        );
